...
class User extends Db{
 ...
 private function setQuestion($question,$con){
   ...
   private function register($con){
    ...
    // Check to ensure no field is empty   email  password  confirm_password  tc
    $tc    = trim($_POST['tc']);
   $cPwd  = isset($_POST['confirm_password'])??trim($_POST['confirm_password']);
    $comparePassword  = $pass<=>$cPwd; 
    if(!isset($tc)){
      echo "Accept T&C please !";
    }else{
        if($comparePassword==0){
            if(!in_array('',$values)){ 
                if(self::checkUserExist($con,$email,"members","email")==false){	
                    ...
                    $statement->bind_param($email,$pass_hash,$date);
